...
| A client could choose its own IP — and rate-limit bucket — via `X-Forwarded-For` | Three attempts, each verified against production. `trustProxy: 1` (last entry): still forgeable. `trustProxy: true` (first entry): still forgeable — Render's docs say the real IP is first, but `/health/whoami` shows the chain is *client-sent, real, hop, hop*. Final: `CLIENT_IP_HEADER=cf-connecting-ip`, the header Cloudflare overwrites, read by `lib/clientIp.ts` for the limiter and the audit log. The lesson is the endpoint: the only way to know what a proxy does is to look from outside |
